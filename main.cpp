...
char buffer[10];
int vehicleX = 300, vehicleY = 90;//used to structure for car racing
int ovehicleX[4], ovehicleY[4];//used to drew other cars in racing
int divx = 250, divy = 4, movd;   //divx  and divy  used to drew the vertex route
...
void drawRoad()
{
    glBegin(GL_QUADS);
        glColor3f(0,0,0);
        glVertex2f(260 - 100, 500);
        glVertex2f(260 - 100, 0);
        glVertex2f(260 + 100, 0);
        glVertex2f(260 + 100, 500);
    glEnd();
}
...
void display()
{
    ...
    glColor3f(1,0.2,0.3);
    drawText("hello in my game", 100, 400);
    drawText("PLAYER", 100, 350);
    drawText("Press 1 to continue", 50, 150);
    glutSwapBuffers();
    }
        else if(screen == 1)
    {
    glClear(GL_COLOR_BUFFER_BIT);
    glColor3f(1,1,0);
    drawText("Use w to accelerate", 100, 400);
    drawText("Use a to move left", 100, 350);
    drawText("Use d to move right", 100, 300);
    drawText("you can use  to control", 100, 250);
    drawText("Press 2 to continue", 50, 100);
    glutSwapBuffers();
    }
    else
    {
